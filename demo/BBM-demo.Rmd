...
# dir_data <- "data_OSF"
dir_project <- "~/Documents/Github/BayesianBrainMapping-priors/demo" # Path to GitHub folder
...
This demo accompanies the manuscript "blah blah blah", which provides a comprehensive introduction to Bayesian Brain Mapping (BBM).  Briefly, there are two steps to performing BBM, both implemented in the `BayesBrainMap` R package: *(1) prior estimation* and *(2) model fitting* (Figure \@ref(fig:model-overview)).

[TO DO]: Replace figure below with one from Overleaf.  Point to the manuscript/figs folder, copy updated file from Overleaf.
...
[TO DO]: Move the appendix stuff into this section.  Use scripts, and source them here, to avoid clutter.
...
In this step, we estimate group-level statistical priors using the `estimate_prior()` function from the `BayesBrainMap` package. We must specify the training data, processing choices, and the template file.
...
-   `scrub` defines volumes to censor and exclude after a target duration. We truncate data to the first 10 minutes (600 seconds), excluding any volumes beyond that point.
...
Here we use the Yeo17 parcellation [@yeo2011organization] as the template. For details on how this parcellation was processed and simplified for use, see [Appendix D](#appendix-parcellations).

#### Run `estimate_prior()`

[TO DO]: Explain that the code below will not be evaluated, but it can be run by pointing to the location of the HCP or other training data on your system.  We already built the priors to be used for model fitting.

Running `estimate_prior()` on the full subject list (\~350 subjects) takes approximately 27 hours and uses 135 GB of memory.

[TO DO]: Check run time and memory requirements with Diego.  He implemented parallel processing so it should be faster than this.
...
In this section, we visualize the Yeo 17 template and the pre-estimated prior maps (mean and variance). We also visualize the functional connectivity prior.

[TO DO]: Read in prior & explain that is what we are doing

### Priors for Spatial Engagement

[TO DO]: Visualize the template. Look at the code in  `08_visualization_Yeo17parcellations.R`. The parcellation used is Yeo17, created in [Appendix D](#appendix-parcellations). This script creates one PNG image per parcel (17 in total), where only the selected parcel is colored and all others are white. [TO DO]: Just visualize the overall parcellation instead of making separate PNGs.

[TO DO]: Use functions from BBM to visualize the prior mean and variance for ONE component. Draw from this info:

Script: `06_visualization_prior.R`

This script loads each estimated prior file from `priors_rds/` and plots both the mean and standard deviation.
...
git clone https://github.com/mandymejia/BayesianBrainMapping-priors.git
cd BayesianBrainMapping-priors
```
...
├── src/
│   ├── 0_setup.R
│   └── 01_fd_time_filtering.R
|   └── ...
├── BayesianBrainMapping-priors.Rmd
└── ...
</pre>
```
...
source(file.path(src_dir,"01_fd_time_filtering.R")) 
```
...
source(file.path(src_dir,"02_unrelated_filtering.R"))
```
...
source(file.path(src_dir,"03_balance_age_sex.R"))
```
...
source(file.path(src_dir,"04_parcellations.R"))
```
