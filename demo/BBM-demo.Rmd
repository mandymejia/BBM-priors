...
This demo accompanies the manuscript "Bayesian brain mapping: population-informed individualized functional topography and connectivity" ([Da Silva Sanchez et al., 2025](https://doi.org/10.48550/arXiv.2602.01551)), which provides a comprehensive introduction to Bayesian Brain Mapping (BBM). Briefly, there are two steps to performing BBM, both implemented in the `BayesBrainMap` R package: *(1) prior estimation* and *(2) model fitting* (Figure \@ref(fig:model-overview)).
...
**Note:** The code below is provided for reference but is not evaluated in this demo. To run it yourself, you must have access to HCP data and update `dir_train` with the path to your HCP data directory. Pre-computed priors are available for download from OSF and are used in the model fitting section below.

Running `estimate_prior()` on the full subject list (~350 subjects) takes approximately 27 hours and uses 135 GB of memory when run sequentially. With parallel processing enabled via `usePar`, runtime is significantly reduced.
...
The Yeo17 parcellation divides the cortex into 17 functional networks. See [Appendix D](#appendix-parcellations) for details on how this parcellation was prepared. The `6_visualization_prior.R` script in `src/` loads each estimated prior file and plots both the mean and standard deviation.

### Priors for Functional Connectivity

The functional connectivity (FC) prior captures the expected correlation structure between networks. The `src/` folder contains visualization scripts for FC priors using both Cholesky and Inverse-Wishart parameterizations.
